...
public:
    ...
    void cameraCallback(Image& image) {
        ...
        static std::atomic<uint64_t> global_sequence_id{0};
        image.sequence_id=global_sequence_id.fetch_add(1,std::memory_order_relaxed);
    ...
    void inferCallback(const InferResult& result) {
        ...
        result_cache[result.sequence_id]=result;
        while (result_cache.find(expected_sequence_id)!=result_cache.end()) 
        {
            InferResult ordered_result=result_cache[expected_sequence_id];
            result_cache.erase(expected_sequence_id);
            track(ordered_result);
...
private:
    ...
    std::map<uint64_t,InferResult> result_cache;
    std::mutex cache_mutex_;
    uint64_t expected_sequence_id=0;
